...
function formatSeoulDateTimeFromEpoch(epochMs: number): string {
  return SEOUL_DATE_TIME_FORMATTER.format(new Date(epochMs)).replace(" ", " ");
}

function formatSeoulDateFromEpoch(epochMs: number): string {
...
function isValidDateParts(year: number, month: number, day: number, hour = 0, minute = 0, second = 0): boolean {
...
function formatDateTime(year: number, month: number, day: number, hour = 0, minute = 0, second = 0): string {
...
function resolveTwoDigitYear(twoDigitYear: number): number {
  const now = new Date();
...
function parseYearFirstDate(value: string): string | null {
  const match = value.match(
    /^(\d{4})[-/ ](\d{1,2})[-/ ](\d{1,2})(?:\s+(?:(AM|PM)\s+)?(\d{1,2}):(\d{2})(?::(\d{2}))?(?:\s+(AM|PM))?)?$/i,
  );
  if (!match) {
    return null;
  }
...
export function normalizeDateTime(rawValue: string): string | null {
...
export function normalizeDateOnly(rawValue: string): string | null {
  const value = cleanDateText(rawValue.trim());
  if (value.length === 0) {
    return null;
  }
...
export function calculateKoreanAge(dateOnly: string, referenceDate = new Date()): number | null {
